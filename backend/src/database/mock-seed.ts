/**
 * 模拟数据种子脚本
 * 生成：用户(每角色2-3个) + 企业进件(20+) + 跟进记录 + 拜访记录 + 推荐记录
 *
 * 用法：cd backend && npx ts-node src/database/mock-seed.ts
 */
...
async function seed() {
  ...
  for (const u of USERS_DATA) {
    ...
    if (!user) {
      ...
      user.name = u.name;
      (user as any).email = u.email || undefined;
      user.isActive = true;
      (user as any).regionId = u.regionId || null;
      user.roles = [role];
  ...
  // 2. 创建企业进件（用 QueryBuilder 直接插入，避免 ORM 类型问题）
  console.log('\n--- Creating intakes ---');
  ...
  for (const c of COMPANIES_DATA) {
    ...
    const result = await intakeRepo
      ...
      .values({
        ...
        applicant: { id: applicant.id } as any,
        assignedTo: assignedTo ? { id: assignedTo.id } as any : undefined,
        createdAt,
        updatedAt: new Date(),
      } as any)
      .execute();
    ...
    intakeMap.set(c.companyName, intakeId as number);
  ...
  for (const c of COMPANIES_DATA) {
    ...
    for (let i = 0; i < fuCountForThis; i++) {
      ...
      fu.operator = { id: operatorId } as any;
      fu.method = tpl.method as any;
      fu.content = tpl.content;
      fu.followDate = followDate;
      fu.result = tpl.result as any;
  ...
  for (const c of COMPANIES_DATA) {
    const intakeId = intakeMap.get(c.companyName);
    if (!intakeId) continue;
    if (!['following', 'approved', 'landed', 'assigned'].includes(c.status)) continue;
    ...
    for (let i = 0; i < visitCountForThis; i++) {
      ...
      visit.operator = { id: operatorId } as any;
  ...
  for (const rp of referralPairs) {
    ...
    ref.referrer = { id: referrer.id } as any;
    ref.type = 'referrer';
