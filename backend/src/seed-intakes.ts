...
async function seed() {
  ...
  const intakeRepo = app.get(getRepositoryToken(Intake));
  const regionRepo = app.get(getRepositoryToken(Region));
  const userRepo = app.get(getRepositoryToken(User));
  const visitRepo = app.get(getRepositoryToken(Visit));
  const followUpRepo = app.get(getRepositoryToken(FollowUp));
  ...
  const regions: Region[] = await regionRepo.find();
  const regionByName: Map<string, Region> = new Map(regions.map((r: Region) => [r.name, r]));
  ...
  dist.forEach((d: any) => console.log(`  ${d.status}: ${d.count}`));
  ...
  for (const d of regDist) {
    const r: Region | undefined = regions.find((x: Region) => x.id === d.rid);
